...
# <li><a href="#updates">Updates from my Tutors</a></li>
...
# 
# This project was a submission for project 2 on my Data Analysis nanodegree program on Udacity

# <a id='updates'></a>
# ## Updates from my tutors
# 
# My tutors recommended a different, and i may say, better way to solve some problems I had with using a function to avoid repitition while plotting graphs. Some of their correction for this submission includes: 
# * Don't hard code values, it could cause error, and it may be difficult to run the codes in the future, should there be a need to update the dataset. In my initial submission, I hard coded values in `code cell [63, 54, and 65]`, as well as in `code cell [56]`.
# * Don't use `iloc[]`, because it is error prone, instead, use the column names.
# 
# **This was their recommendation for `code cell [63, 54, and 65]`:**
# I was trying to create a function that plots the first five rows of a data set in a bar chat. The funtion needed to be reusable for other datasets with similar formats/structures. See `code cell [63, 54, and 65]` for what i did. Below is their recommendation
# >**Instead of using a function that specifies the years, you can plot the dataframe directly:**
#     >
#     * `#creating a function that 'uses all columns except'`
#     >
#     * `select_columns = [x for x in top_prod.columns if x not in ['country mean', 'product levels']]`
#     >
#     * `#select only the required column ` 
#     >
#     * `top_prod[select_columns].head(5).T`
# 
# 
# then, after this, we plot the graphs.
# 
# >**Plotting Graphs**:
#     > `top_prod[select_columns].head(5).T.plot.bar(subplots=True,figsize=14,14,rot=0, legend=False,width=0.8,edgecolor="black");`
#     >
#     * `plt.tight_layout()`
# 
# 
# **This was their recommendation for `code cell [56]`:**
# I was trying to specify bin edges for my work. This was what i did before:
# 
# `bin_edges = [ 1.000000, 3010.375, 13355.00, 58907.50, 1661000.00 ]`
# 
# This was their comment: *"You want your code to be applicable to all datasets with the same structure (for example, if next year you want to run your code on an updated dataset). hard-coding values will not allow you to do this (as those values will remain constant regardless of how the data changes - you would have to update them manually, which is inefficient):"
# 
# My tutor recommended a way to create `bin_edges` without hardcoding the values. 
# >Here is their recommendation:
#     >
#     >`bin_edges = df_energy_prod['country_mean'].describe().loc[['min', '25%', '50%', '75%', 'max']].values`
#     >
#     * `bin_edges`
# 
# 
...
#bin_edges = [ 1.000000, 3010.375, 13355.00, 58907.50, 1661000.00 ]
#with recommendations from my tutor, here is my update. See `Updates from my tutor` cell
